...
Curriculum training is still not authorized. The saved physical tail state was
generated by the earlier behavior policy, not by the updated actor, so this is
an explicit off-policy start distribution. The remaining gate is a formal
sampler that uses this contract while keeping the declared `4 worlds / 8 epochs
/ 1280 PPO samples` budget and leaves CPU endpoint-20 `40/40` acceptance
unchanged.
