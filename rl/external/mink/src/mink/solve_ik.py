...
def solve_ik(
    ...
    **kwargs,
    ...
    # With no objectives, the unique minimum-norm command is zero.  Avoid
    # sending an empty QP to solver backends that reject zero-row problems.
    if not tasks and not constraints:
        return np.zeros(configuration.nv, dtype=np.float64)
