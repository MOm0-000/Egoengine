...
class Configuration:
    ...
    * Integrating velocities to update configurations.
    ...
    def get_inertia_matrix(self) -> np.ndarray:
        ...
        # The Python binding accepts (model, data, dense). Older bindings
        # exposed the sparse matrix as data.qM, but current MuJoCo keeps it
        # internal; pass the data object so both layouts remain supported.
        try:
