...
class CollisionAvoidanceLimit(Limit):
    ...
    def __init__(
        ...
        include_explicit_pairs: bool = False,
    ):
        ...
        Args:
            ...
            include_explicit_pairs: Honor model.contact pairs present in the
                requested groups even for adjacent bodies or disabled masks.
                Same/welded-body pairs remain excluded. Default filtering is
                unchanged; this does not add pairs absent from geom_pairs.
        """
        self.model = model
        self.gain = gain
        ...
        self._explicit_pairs = (
            {tuple(sorted((int(a), int(b))))
             for a, b in zip(model.pair_geom1, model.pair_geom2)}
            if include_explicit_pairs else set()
        )
    ...
    def _construct_geom_id_pairs(self, geom_pairs):
        ...
        for id_pair in self._collision_pairs_to_geom_id_pairs(geom_pairs):
            for geom_a, geom_b in itertools.product(*id_pair):
                ...
                explicit = tuple(sorted((geom_a, geom_b))) in self._explicit_pairs
                if weld_body_cond and (explicit or (parent_child_cond and contype_conaffinity_cond)):
