...
BASELINE = ROOT / "runs/taco_pour_bimanual_mano_fk_bilateral_guard_v1"
...
def compare():
    ...
    inputs = [artifact(p) for p in (SCENE, candidate_scene, BASELINE / "robot_reference.npz",
        ROOT / "runs/taco_pour_initialization_preflight_bilateral_guard_v1/report.json",
    ...
    preflight = json.loads((ROOT / "runs/taco_pour_initialization_preflight_bilateral_guard_v1/report.json").read_text())
...
def screen_hand_geometry(labels=("mesh_hands", "budgeted_palms"), destination=OUTPUT / "hand_geometry_screen.json", models=None):
    if destination is not None and destination.exists():
        raise FileExistsError(destination)
    report = json.loads((ROOT / "runs/taco_pour_initialization_preflight_bilateral_guard_v1/report.json").read_text())
...
def compare_refinement():
    ...
    result = dict(status="targeted_shape_refinement_not_formal_model", dynamics=unchanged,
        ...
        native_preflight=artifact(ROOT / "runs/taco_pour_initialization_preflight_bilateral_guard_v1/report.json"),
        code=artifact(Path(__file__)),
        distance_code=artifact(ROOT / "src/egoengine_repro/retarget/mesh_distance.py"),
