"""HaMeR adapter producing the same frozen monocular hand artifact schema as WiLoR.

This is the H1 candidate in the upstream ablation plan. It replaces only the
per-view monocular hand model (WiLoR -> HaMeR), keeps the existing calibrated
stereo frames, and uses the same YOLO hand detector so the comparison is
isolated to the MANO estimator.
"""
...
def _camera_translation(pred_cam, box_center, box_size, K):
    """Convert HaMeR crop weak-perspective camera to the run's full camera."""
    import torch
...
def run(args: argparse.Namespace) -> Path:
    hamer_root = Path(__file__).resolve().parents[2] / "third_party/hamer"
    if not hamer_root.is_dir():
        raise RuntimeError(f"HaMeR root not found: {hamer_root}")
    ...
    # Keep the default cache relative to the external repo, not the current cwd.
    hamer.configs.CACHE_DIR_HAMER = str(hamer_root / "_DATA")

    if args.device == "cuda" and not torch.cuda.is_available():
        raise RuntimeError("HaMeR requested CUDA but no device is visible")
    ...
    default_output = run_dir / ("hands_hamer" if camera_view == "left" else "hands_right_hamer")
    output_dir = args.output_dir.resolve() if args.output_dir else default_output
    metadata_path = output_dir / "metadata.json"
    if metadata_path.exists() and not args.overwrite:
        raise FileExistsError(f"HaMeR output exists: {output_dir}")
    output_dir.mkdir(parents=True, exist_ok=True)

    model, model_cfg = load_hamer(str(args.checkpoint.resolve()))
    detector = YOLO(str(args.detector_checkpoint.resolve()))
    ...
    for frame_offset, row in enumerate(rows):
        ...
        if boxes:
            ...
            dataset = ViTDetDataset(
                model_cfg, image, box_array, right_array, rescale_factor=args.rescale_factor,
            )
    ...
    artifact_path = output_dir / "hamer_raw.npz"
    np.savez_compressed(
        artifact_path,
    ...
    overlay_path = output_dir / "hamer_overlay.mp4"
    ...
    metadata = {
        ...
        "camera_translation_conversion": "HaMeR weak-perspective crop to full OpenCV camera",
        "video_encoding": VIDEO_ENCODING,
        "hand_order": ["left", "right"],
        "frame_count": t,
        "valid_rate_left": float(np.mean(valid[:, 0])),
        "valid_rate_right": float(np.mean(valid[:, 1])),
        "shared_beta_initial": {
        ...
        "outputs": ["hamer_raw.npz", "hamer_overlay.mp4"],
    }
